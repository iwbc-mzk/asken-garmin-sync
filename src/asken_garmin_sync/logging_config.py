...
def configure_logging(level: int = logging.INFO) -> None:
    ...
    ログレベルは引数 level（デフォルト: INFO）で設定する。
    ...
    if root.handlers:
        # 既存ハンドラーのフォーマッターと個別レベルを JSON / 指定レベルに置き換える。
        # Lambda の LambdaLoggerHandler はハンドラー固有レベルを持つため、
        # root レベルだけでなく各ハンドラーのレベルも明示的に設定する必要がある。
